...
import { FaRegLightbulb, FaExclamationTriangle, FaCar, FaBus, FaWalking, FaBicycle } from "react-icons/fa";
        ...
        <>
            {
                (Object.keys(currentRoute).length === 0) ?
                    <div className="impactNoticeBox">
                        <p><FaExclamationTriangle />  Create a route to see how much it will impact the environment!</p>
                    </div> :
                                ...
                                getOtherTransport(currentRoute["travelMode"]).map(mode => (
                                    (getImpact(mode).carbonEmissions === 0) ? (<p>Generating...</p>) :
                                        (<p><span>
                                            ...
                                            <span className="otherModeHeader"> {mode}</span>: {getImpact(mode).carbonEmissions.toFixed(2)} kg CO2 {(getStatComparison(currentRoute["travelMode"], mode) > 0 ?
                                        ...
                                        <br/><span className="otherModeDuration">({secondsToHms(getImpact(mode).duration)})</span>
                                        </p>)
                                ))

                            }
                        </div>
                        {(!currentRoute["optimizeWaypoints"]) ? <p className="optimiseNotice"><FaRegLightbulb /> Optimise your route now for greater efficiency!</p> : <></>}
                    </div>
            }
        </>
